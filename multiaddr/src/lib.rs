...
impl TryFrom<&str> for Onion3Addr<'_> {
    type Error = Error;

    fn try_from(s: &str) -> Result<Self, Error> {
        let addr: Vec<&str> = s.split(':').collect();
        if addr.len() != 2 {
            return Err(Error::InvalidOnion3Addr);
        }
        ...
        let onion_host_bytes = base32::decode(
            Alphabet::RFC4648 { padding: false },
            &addr[0].to_uppercase(),
        )
        .ok_or_else(|| Error::InvalidOnion3Addr)?;

        // Onion port number
        let port: u16 = addr[1].parse().map_err(|_| Error::InvalidOnion3Addr)?;

        if port == 0 {
            return Err(Error::InvalidOnion3Addr);
        }

        let onion_host_array: &[u8; 35] = onion_host_bytes[0..35]
            .try_into()
            .map_err(|_| Error::InvalidOnion3Addr)?;

        Ok(Onion3Addr(Cow::Borrowed(onion_host_array), port))
    }
}
